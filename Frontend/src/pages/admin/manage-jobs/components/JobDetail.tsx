...
const JobDetail: React.FC<Props> = ({ jobId, open, onClose }) => {
  const navigate = useNavigate();

  const { data } = useQuery({
    queryKey: ['getJobByAdmin', jobId],
    queryFn: () => getJobByAdmin(Number(jobId)),
        ...
        {data && (
          <div className='mx-auto max-w-4xl'>
            <div className='rounded-2xl border-white/20 bg-white/80 p-6 shadow-xl backdrop-blur-sm'>
              ...
              <div className='mt-3 mb-6 border-b border-b-gray-200' />

              {/* Main content */}
              <div className=''>
                {/* Hero section */}
                <div className='relative flex flex-col gap-3 border-gray-100 bg-white px-0 pb-8 sm:gap-4'>
                    ...
                    {data.status === 'PENDING' && (
                      <>
                        <ButtonForm
                          ...
                          onClick={() =>
                            mutation.mutate({ status: 'REJECTED' as unknown as JobStatus, jobId: +data.id })
                          }
                        />
                        <ButtonForm
                          name='Duyệt'
